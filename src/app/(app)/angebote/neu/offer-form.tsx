...
// Interaktives Angebots-Formular (laeuft im Browser).
...
export default function OfferForm({
  ...
  function onSelectPackage(key: number, packageId: string) {
    ...
    if (pkg) {
      ...
      // "Freie Position" gewaehlt: nur die Zuordnung loeschen
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    // Positionen aufbereiten und leere herausfiltern
    const items = rows
      .map((r) => ({
        label: r.label.trim(),
    ...
    // An den Server schicken (createOffer leitet bei Erfolg automatisch weiter)
    startTransition(async () => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Beim Speichern ist ein Fehler aufgetreten."
        );
      }
    });
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {/* --- Angebotsdaten --- */}
      <section className="glass rounded-2xl p-5">
        <h2 className="mb-4 font-semibold">Angebotsdaten</h2>
      ...
      <section className="glass rounded-2xl p-5">
        ...
        <div className="mt-5 flex justify-end border-t border-line pt-4">
          <div className="text-right">
            <div className="text-sm text-muted">Gesamtsumme (netto)</div>
      ...
      <section className="glass rounded-2xl p-5">
        ...
        <textarea
          ...
          placeholder="Individuelle Anmerkungen, die im Angebot erscheinen sollen."
        />
      </section>
      ...
      <div className="flex justify-end gap-3">
        ...
        >
          {isPending ? "Wird gespeichert …" : "Angebot speichern & PDF erstellen"}
        </button>
      </div>
    </form>
  );
